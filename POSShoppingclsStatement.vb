...
''' </summary>
Public Class POSShoppingclsStatement
    Implements IDisposable

#Region " Variables & Constants "

    Private m_DataShoppingSet As New DataSetShop 'Database of the loaded statement
    ...
    Private WithEvents m_statementFiles As POSShoppingStatementFiles
...
#End Region
    ...
    Public Sub New(statementFiles_ As POSShoppingStatementFiles, PDFMetaData_Passed_ As pdfLib.PDFMetaData _
...
#Region " File Handler "
    ...
    Function readRecords() As Boolean
        ...
        Try
            ...
            Dim Bal As New Structures.POSSCSTBalance
            If Me.m_Customer.NoOfShpWal.Trim <> "" AndAlso IsNumeric(m_Customer.NoOfShpWal) AndAlso CInt(m_Customer.NoOfShpWal) > 0 Then
                count = oDBProvider.getCustomerWallets(m_Customer.CusNum, 1, Records)
                If count > 0 Then
                ...
                End If
                Record = Nothing
                count = oDBProvider.ShoppingCardTransactionsExtract(m_Customer.CusNum, m_statementFiles.StatementMonth, m_statementFiles.StatementYear, Records)
                ...
                Record = Nothing
                'count = oDBProvider.BalanceExtract(m_Customer.CusNum, m_statementFiles.StatementMonth, m_statementFiles.StatementYear, Records)
                'If count > 0 Then
                '    ReDim m_TravelCusStm.Balance(count - 1)
                '    For i As Integer = 0 To Records.Tables(0).Rows.Count - 1
                '        lRow = Records.Tables(0).Rows(i)
                '        With Bal
                '            .clear()
                '            .CusNum = lRow(0).ToString.Trim
                '            .WalId = lRow(1).ToString.Trim
                '            .Ccy = lRow(2).ToString.Trim
                '            .AccountId = lRow(3).ToString.Trim
                '            .AvailableBalance = lRow(4).ToString.Trim
                '            .LedgerBalance = lRow(5).ToString.Trim
                '            .PreviousMonthEnd = Convert.ToDateTime(lRow(6))
                '        End With
                '        m_TravelCusStm.Balance(i) = Bal
                '    Next
                'End If
            End If
            ...
            Return True
        Catch ex As Exception
            ServicesUti.Uti.handleMessage(&H89010000, SIBL0100.EventType.Critical, "An error has occurred while read Shopping card transactions.", "Unknown Error : " & ex.Message, ex, ShowOn:=False)
            Return False
        Finally
    ...
    Private Function checkStatementHeaderFooter() As Boolean
        ...
        Try
            ...
            If sCheckDataAvailability <> String.Empty Then
                ...
                ServicesUti.Uti.handleMessage(&H89010000, SIBL0100.EventType.Critical, sStat & ", This statement Header record does not have data in field(s) (" & sCheckDataAvailability & ")", ShowOn:=False)
                Return False
            End If
            '---------------------------------
...
#Region " Statement File Processing "
    ...
    Sub AskUser(Optional ByVal sPassedError_ As String = Nothing)
        If Me._StatementStatus = frmFileErrorB.enumFileErrorUserSelection.CrystalReportsError Then
            Exit Sub
        End If
        Me._StatementStatus = frmFileErrorB.enumFileErrorUserSelection.ResumeFile 'Me.m_statementFiles.AskUser(m_fileProcessStartDate, m_totalNumberOfStatements, m_CurrentStatementNumber, sPassedError_)
    End Sub
...
FileRestart:
            ...
            Grid_MakeNewRow("Shopping Card, CIF:" & Me.CustomerNumber & " Time: " & Me.m_fileProcessStartDate.ToString(ServicesUti.GlobalVars.C_UI_DateFormat))
            Application.DoEvents()
            'Return True
            ...
            ServicesUti.Uti.handleMessage(&H89010000, SIBL0100.EventType.Information, String.Format("Processing shopping statement #[{0}], for Acc #[{1}]", Me.m_CurrentStatementNumber.ToString("##,#0"), m_TravelCusStm.SHD.CusNum), LogLevel:=3, ShowOn:=False)
            ...
            bRetValue = GenerateShoppingCardDataSet()
            ...
            Logger.LoggerClass.Singleton.LogInfo(0, "statement -> Full statement Started, Started...", 4)

            ' Reading Statement from file
            Dim isGetStatementRecords As Boolean = False
            Dim dGetStatementRecords As Date
            Logger.LoggerClass.Singleton.LogInfo(0, "statement -> Reading Statement from file, Started...", 4)
            dGetStatementRecords = Now
    ...
    Sub processAbort()
        ...
        'If m_DBParentFileNo = 0 And m_DBExecutionNo = 0 Then
        '    lDBFileNumber = m_statementFiles.DBFileNumber
        '    lDBFileExecutionNumber = m_statementFiles.DBFileExecutionNumber
        'Else
        '    lDBFileNumber = m_DBParentFileNo
        '    lDBFileExecutionNumber = m_DBExecutionNo
        '    m_statementFiles.StatementsLog.fileStats_DeleteAll(m_DBParentFileNo, m_DBExecutionNo, False)
        'End If
    ...
    Sub processRestart(Optional deleteParent_ As Boolean = False)
        ...
        'Else
        '    m_statementFiles.StatementsLog.fileStats_DeleteAll(m_DBParentFileNo, m_DBExecutionNo, deleteParent_)
        'End If
        ...
        'Me.m_statementFiles.FilesProcessSummary.CurrentFileParentNo = Me.m_statementFiles.DBFileNumber
    End Sub
    ...
    Sub processCrystalReportsError()
        ...
        ServicesUti.Uti.handleMessage(&H89010000, SIBL0100.EventType.Critical, sMsg, , Nothing, GenerateAIDS:=False, ShowOn:=False)
...
#Region " Generate Statement "
    ...
    ''' <remarks>
    ''' </remarks>
    Public Function GenerateShoppingCardDataSet() As Boolean

        Dim StmHdr() As DataSetShop.HRDRow
        ...
        Try
            ...
            Me.m_DataShoppingSet.Clear()
            ReDim CurrenctCcyTrx(0)
            m_ForceAddressLeftAlign = True
            With m_TravelCusStm
                Dim sDate As String
                sDate = USFormatDate(gCurrentMonthEnd, "yyyy-MM-dd")
                ...
                End If
                StmHdr(0) = m_DataShoppingSet.HRD.AddHRDRow(0, .SHD.CusNum.Trim, sDate, AddLine1, AddLine2, AddLine3, AddLine4, "", Flgs)

                'If m_TravelCusStm.SHD.CusLng = "A" Then
                '    StmHdr(0) = m_DataShoppingSet.HRD.AddHRDRow(0, .SHD.CusNum.Trim, sDate, .SHD.CusArAddLin1, .SHD.CusArAddLin2, .SHD.CusArAddLin3, .SHD.CusArAddLin4, "", Flgs)
                'Else
                '    StmHdr(0) = m_DataShoppingSet.HRD.AddHRDRow(0, .SHD.CusNum.Trim, sDate, .SHD.CusEnAddLin1, .SHD.CusEnAddLin2, .SHD.CusEnAddLin3, .SHD.CusEnAddLin4, "", Flgs)
                'End If
                WalID = .WHD(0).WalId
                'If m_TravelCusStm.SHD.CusLng = "A" Then
                '    StmHdr(0) = m_DataShoppingSet.HRD.AddHRDRow(0, .SHD.CusNum.Trim, sDate, .SHD.CusArNam1 & " " & .SHD.CusArNam2 & " " & .SHD.CusArNam3 & " " & .SHD.CusArNam4, .SHD.CusArAddLin1, .SHD.CusArAddLin2, .SHD.CusArAddLin3, .SHD.CusArAddLin4, Flgs)
                'Else
                '    StmHdr(0) = m_DataShoppingSet.HRD.AddHRDRow(0, .SHD.CusNum.Trim, sDate, .SHD.CusEnNam1 & " " & .SHD.CusEnNam2 & " " & .SHD.CusEnNam3 & " " & .SHD.CusEnNam4, .SHD.CusEnAddLin1, .SHD.CusEnAddLin2, .SHD.CusEnAddLin3, .SHD.CusEnAddLin4, Flgs)
                'End If

                lBalance = New Structures.POSSCSTBalance
                lBalance.clear()
                If INIParameters.Singlton.Glob.ShowBalances Then
                    If GetCcyBalance(.Balance, lBalance, "682", "0") Then
                        'balance 
                        ' Balance Brought Forward
                        tmpOpnBal = FormatBalance1(lBalance.LedgerBalance, 2, "SAR") & " Cr" 'FormatAmount(lBalance.LedgerBalance)
                        tmpOpnBalDsc = "Balance at end of last statement"
                        m_DataShoppingSet.TRX.AddTRXRow(StmHdr(0), USFormatDate(startP, "dd-MMM-yy"), "", tmpOpnBalDsc, "", tmpOpnBal)
                        'get currency trx
                    Else
                        tmpOpnBal = FormatBalance1("0", 2, "SAR") & " Cr" 'FormatAmount(lBalance.LedgerBalance)
                        tmpOpnBalDsc = "Balance at end of last statement"
                        m_DataShoppingSet.TRX.AddTRXRow(StmHdr(0), USFormatDate(startP, "dd-MMM-yy"), "", tmpOpnBalDsc, "", tmpOpnBal)
                    End If
                End If
                ...
                If GetCcyTransactions(.TRX, CurrenctCcyTrx, "682", WalID) Then
                    ...
                    If Not CurrenctCcyTrx Is Nothing AndAlso CurrenctCcyTrx.Length > 0 Then
                        For k As Integer = 0 To CurrenctCcyTrx.Length - 1
                            ...
                            m_DataShoppingSet.TRX.AddTRXRow(StmHdr(0), USFormatDate(CurrenctCcyTrx(k).TrxDatetime, "dd-MMM-yy"), USFormatDate(CurrenctCcyTrx(k).PostDatetime, "dd-MMM-yy"), FormatNarrations(CurrenctCcyTrx(k)), tmptrxAmunt, FormatBalance(CurrenctCcyTrx(k).LB, 2, CurrenctCcyTrx(k).SettleCurrencyCode))
                            lCount = lCount + 1
                        Next
                ...
                Else
                    If lCount = 0 Then
                        m_DataShoppingSet.TRX.AddTRXRow(StmHdr(0), "No transactions available to show", "", "", "", "")
                    End If
                End If
        ...
        Catch ex As Exception
            ServicesUti.Uti.handleMessage(&H89010000, SIBL0100.EventType.Critical, "Error in GenerateXMLDataSet.Because an exception happened.", ex.InnerException.ToString, ShowOn:=False)
        Finally
    ...
    Private Function GetCcyTransactions(ByRef AllTrx() As Structures.POSSCSTCusStm_Trx, ByRef CcyTrx() As Structures.POSSCSTCusStm_Trx, CcyCde As String, WalId As String) As Boolean
        ...
        Else
            ...
            For i = 0 To AllTrx.Length - 1
                If AllTrx(i).SettleISOCurrencyCode = CcyCde Then
                    CcyTrx(idx) = AllTrx(i)
                    idx += 1
                End If
            Next
    ...
    Private Function FormatNarrations(ByVal TrxDsc As Structures.POSSCSTCusStm_Trx) As String
        ...
        Catch ex As Exception
            ServicesUti.Uti.handleMessage(&H89010000, SIBL0100.EventType.Critical, "An error has occurred while Format Narrations.", String.Empty, ex, LogLevel:=1, ShowOn:=False)
        End Try
    ...
    Public Overridable Function GenerateStatement() As Boolean
        ...
        Try
            ...
            sRank = m_TravelCusStm.SHD.CusSeg
            ...
            rptName = String.Format(POSStatementFiles.C_SCSTReportTemplateName, StmLng, sRank, "01")
            ...
            End If
            If m_DataShoppingSet Is Nothing Then
                ServicesUti.Uti.handleMessage(&H89010000, SIBL0100.EventType.Critical, "m_DataSet is null.")
            End If
            With oCrystalRep
                .unLoadReport()
                .SetDataSource(m_DataShoppingSet)
            ...
            If Not isExported Then
                ...
                'Me._StatementStatus = frmFileErrorB.enumFileErrorUserSelection.CrystalReportsError
                'EventHandler(&H89010000, SIBL0100.EventType.Critical, "System is out of resources.", , Nothing, FileName:=FileName)
                'isExported = False
            End If
        ...
        Catch ex As Exception
            ...
            If ex.Message.Contains("Report Application Server failed") Then
                ...
                ServicesUti.Uti.handleMessage(&H89010000, SIBL0100.EventType.Critical, "System is out of resources.", , DirectCast(ex, Exception))
            Else
                'ServicesUti.Uti.handleMessage(&H89010000, SIBL0100.EventType.Critical, String.Format("Line #{0:##,#0} An Unexpected error has occurred, at GenerateStatement.", fileReachedLine), , ex, FileName:=fileName)
            End If
        Finally
    ...
    ''' <remarks>
    ''' </remarks>
    Private Function GenerateOutputPath() As Boolean
...
#Region " Statement Post Processing "
    ...
    Private Function ExportToPDF(ByVal oCrystalRep As CCrystalReport, ByVal pdfFilePath As String) As Boolean
        ...
        Try
            ...
            With oCrystalRep
                ...
                If isExported Then
                    ServicesUti.Uti.handleMessage(&H89010000, SIBL0100.EventType.Information, "statement -> Exported to: " & pdfFilePath, String.Empty, , LogLevel:=8, GenerateAIDS:=False, ShowOn:=False)
    ...
    ''' <summary>
    ''' The function print the summary report for the processed file.
    ''' </summary>
    ''' <remarks>
    ''' Created : 2011-02-22 DK
    ''' </remarks>
    Public Sub GenerateSummaryReport1()
        Dim sPdfFilePath As String
        Dim isExported As Boolean = False
        Dim iTmp As Long
        Dim sTmpNum As String
        Dim repSummery As CCrystalReport = Nothing

        Try
            Logger.LoggerClass.Singleton.LogInfo(0, "Started", 9)

            'If repSummery Is Nothing Then
            '    repSummery = New CCrystalReport(gCrystalViewer)
            '    repSummery.OpenReport(m_template_Summary)
            'End If

            'repSummery = Me.Reports.getReport(StatementFiles.C_ReportSummaryTemplateName)

            'sPdfFilePath = IO.Path.Combine(ServicesUti.AppPaths.singleton.LogFilePath, fileName & ".VDXF0300.PDF")
            'clearReport(repSummery)

            'With repSummery
            '    .SetFormulaField("InpFil", String.Format("'{0}'", fileName))
            '    .SetFormulaField("StrTim", String.Format("'{0}'", m_StatementSummary.StartTime.ToString(ServicesUti.GlobalVars.C_UI_DateFormat)))
            '    .SetFormulaField("EndTim", String.Format("'{0}'", m_StatementSummary.EndTime.ToString(ServicesUti.GlobalVars.C_UI_DateFormat)))
            '    .SetFormulaField("TotDur", String.Format("'{0}'", m_StatementSummary.Duration))

            '    m_StatementSummary.FileTotalLines = FileTotalLines
            '    .SetFormulaField("TotRec", String.Format("'{0} records processed.'", m_StatementSummary.FileTotalLines.ToString("##,##")))

            '    iTmp = m_StatementSummary.Total : If iTmp = 0 Then sTmpNum = "0" Else sTmpNum = iTmp.ToString("##,##")
            '    .SetFormulaField("TotStm", String.Format("'{0} statements processed.'", sTmpNum))


            '    iTmp = m_StatementSummary.Print : If iTmp = 0 Then sTmpNum = "0" Else sTmpNum = iTmp.ToString("##,##")
            '    .SetFormulaField("PrtStm", String.Format("'{0} statements printed.'", sTmpNum))

            '    iTmp = m_StatementSummary.Email : If iTmp = 0 Then sTmpNum = "0" Else sTmpNum = iTmp.ToString("##,##")
            '    .SetFormulaField("EmlStm", String.Format("'{0} statements emailed.'", sTmpNum))

            '    iTmp = m_StatementSummary.Archived : If iTmp = 0 Then sTmpNum = "0" Else sTmpNum = iTmp.ToString("##,##")
            '    .SetFormulaField("StatmentsExported", String.Format("'{0} statements exported to pdf.'", sTmpNum))

            '    If m_StatementSummary.FileSize = 0.0 Then sTmpNum = "0" Else sTmpNum = m_StatementSummary.FileSize.ToString("##,#0.00")
            '    .SetFormulaField("InpFilSize", String.Format("'{0} MB'", sTmpNum))

            '    If Me._StatementStatus = frmFileErrorB.enumFileErrorUserSelection.CompletedSuccessfully Then
            '        .SetFormulaField("PrsSta", "'Completed'")
            '        .SetFormulaField("PrsErr", "'None'")
            '    Else
            '        .SetFormulaField("PrsSta", "'Failed'")
            '        .SetFormulaField("PrsErr", String.Format("'{0}'", gLastErrorMsg.Replace("'", String.Empty)))
            '    End If

            '    isExported = .ExportReportTo(CrystalDecisions.[Shared].ExportFormatType.PortableDocFormat, sPdfFilePath)
            'End With
        ...
        Catch ex As CrystalDecisions.CrystalReports.Engine.LoadSaveReportException
            ...
            ServicesUti.Uti.handleMessage(&H89010000, SIBL0100.EventType.Critical, "System is out of resources.", , DirectCast(ex, Exception))
        Catch ex As Exception
            ServicesUti.Uti.handleMessage(&H89010000, SIBL0100.EventType.Critical, "An error has occurred while GenerateSummaryReport:" & ex.Message, String.Empty, ex, LogLevel:=1)
        Finally
            'repSummery.CloseReport()
