...
Public Class frmAlertServices
    ...
    Private chkAlertEmailOldState As Boolean
    Private chkAlertMobileOldState As Boolean
...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.pnlButtons.Location = New System.Drawing.Point(8, 369)
        ...
        'btnTstEml
        '
        Me.btnTstEml.Location = New System.Drawing.Point(408, 79)
        ...
        'chkMobPhn_UpdAll
        '
        Me.chkMobPhn_UpdAll.Location = New System.Drawing.Point(412, 103)
        ...
        'chkEmlAdd_UpdAll
        '
        Me.chkEmlAdd_UpdAll.Location = New System.Drawing.Point(412, 119)
        ...
        Me.lblMessage.Location = New System.Drawing.Point(18, 306)
        ...
        'lblMessage2
        '
        Me.lblMessage2.ForeColor = System.Drawing.Color.Navy
        Me.lblMessage2.Location = New System.Drawing.Point(18, 331)
        Me.lblMessage2.Name = "lblMessage2"
        Me.lblMessage2.Size = New System.Drawing.Size(582, 31)
        Me.lblMessage2.TabIndex = 70
        Me.lblMessage2.TextAlign = System.Drawing.ContentAlignment.BottomLeft
        '
        'frmAlertServices
        '
        Me.AutoScaleBaseSize = New System.Drawing.Size(5, 13)
        Me.ClientSize = New System.Drawing.Size(623, 409)
...
#End Region
    ...
    Private Sub btnClose_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnClose.Click
        ...
        ' If the button text is "Close", simply close the form
        ...
        ' If the button text is "Cancel", check for unsaved changes

        If btnClose.Text = "&Cancel" Then

            If m_TrackChanges.isChanged Then

                ' Prompt the user to confirm if they want to cancel without submitting changes
                ...
                ' If the user chooses "No", return to the form without closing it
            ...
            ' Restore the original data if the user cancels without submitting changes
            ...
            With MsgRspDat
                ...
                cmbStatus.SelectedIndex = CInt(If(MsgRspDat.AltRegFlg = "Y", 1, 0))

            End With

        End If

        ' Clear tracking changes and reset the form
    ...
    Private Sub ApproveChanges()
        ...
        If isSuccess Then
            ...
            'RemoveExistingArrow(cmbPreferedLanguageOld)
            'RemoveExistingArrow(cmbStatusOld)
            'RemoveExistingArrow(txtEmailAddressOld)
            'RemoveExistingArrow(txtMobilePhoneOld)
    ...
    Private Sub EnableFormControls(ByVal isEnabled As Boolean)
        ' Enable or disable the form and buttons
    ...
    Private Function SubmitToHost() As Boolean
        ...
        Try
            ...
            If IsValidMid(MsgID) Then
                If Not (GetMessage_ModifyAlertServices(MsgID, Nothing)) Then
    ...
    Private Function LoadNewValues(ByVal customerNumber As String) As Task
        ...
        Dim sql As String = "SELECT OldMobilePhone, NewMobilePhone, OldEmailAddress,  NewEmailAddress, " &
                            ...
                            "FROM AlertServiceChanges WHERE CusNum = @CusNum AND Status = 'Approved' order by RequestDate desc"
    ...
    Private Sub GetLastUpdateDetails(ByVal customerNumber As String)

        ' Define SQL query to get the last approved or rejected changes

        Dim sql As String = "SELECT TOP 1 MakerID, RequestDate, CheckerID, ApprovedDate, RejectedDate, Status " &
                            "FROM AlertServiceChanges WHERE CusNum = @CusNum AND Status IN ('Approved', 'Rejected') " &
                            "ORDER BY ApprovedDate DESC, RejectedDate DESC"
        ...
        Using conn As New SqlConnection(connectionString)
            ...
            Using cmd As New SqlCommand(sql, conn)
                ...
                Using reader As SqlDataReader = cmd.ExecuteReader()
                    ...
                    If reader.Read() Then
                        ' Retrieve and format the last update details
                        Dim modifiedBy As String = reader("MakerID").ToString()
                        Dim modifiedDate As String = DateTime.Parse(reader("RequestDate").ToString()) _
                                                    .ToString("dd-MMM-yyyy HH:mm:ss", _
                                                              System.Globalization.CultureInfo.InvariantCulture)

                        Dim actionBy As String = reader("CheckerID").ToString()
                        Dim status As String = reader("Status").ToString()

                        ' Determine whether to use ApprovedDate or RejectedDate
                        Dim actionDate As String = String.Empty
                        If status = "Approved" Then
                            actionDate = DateTime.Parse(reader("ApprovedDate").ToString()) _
                                         .ToString("dd-MMM-yyyy HH:mm:ss", _
                                                   System.Globalization.CultureInfo.InvariantCulture)
                        ElseIf status = "Rejected" Then
                            actionDate = DateTime.Parse(reader("RejectedDate").ToString()) _
                                         .ToString("dd-MMM-yyyy HH:mm:ss", _
                                                   System.Globalization.CultureInfo.InvariantCulture)
                        End If

                        ' Prepare the message based on the status
                        If status = "Approved" Then
                            lblMessage2.Text = String.Format("Last update was made by {0} on {1} and approved by {2} on {3}.",
                                                             modifiedBy, modifiedDate, actionBy, actionDate)
                        ElseIf status = "Rejected" Then
                            lblMessage2.Text = String.Format("Last update was made by {0} on {1} and rejected by {2} on {3}.",
                                                             modifiedBy, modifiedDate, actionBy, actionDate)
                        End If

                    Else

                        ' If no previous changes found

                        lblMessage2.Text = "No previous approved or rejected changes found for this customer."
    ...
    Private Sub SaveMakerChanges()
        Try
            ...
            Dim oldMobilePhone As String = If(String.IsNullOrEmpty(txtMobilePhoneOld.Text), Nothing, txtMobilePhoneOld.Text)

            Dim newMobilePhone As String = If(String.IsNullOrEmpty(txtMobilePhone.Text), Nothing, txtMobilePhone.Text)

            Dim oldEmailAddress As String = If(String.IsNullOrEmpty(txtEmailAddressOld.Text), Nothing, txtEmailAddressOld.Text)

            Dim newEmailAddress As String = If(String.IsNullOrEmpty(txtEmailAddress.Text), Nothing, txtEmailAddress.Text)

            Dim oldLanguage As String = If(cmbPreferedLanguageOld.SelectedItem Is Nothing, Nothing, cmbPreferedLanguageOld.SelectedItem.ToString())

            Dim newLanguage As String = If(cmbPreferedLanguage.SelectedItem Is Nothing, Nothing, cmbPreferedLanguage.SelectedItem.ToString())

            Dim oldStatus As String = If(cmbStatusOld.SelectedItem Is Nothing, Nothing, cmbStatusOld.SelectedItem.ToString())

            Dim newStatus As String = If(cmbStatus.SelectedItem Is Nothing, Nothing, cmbStatus.SelectedItem.ToString())
            ...
            Using conn As New SqlConnection(connectionString)
                Using cmd As New SqlCommand(sql, conn)
                    cmd.Parameters.AddWithValue("@AlertTrxRef", transactionRef)
                    cmd.Parameters.AddWithValue("@CusNum", txtCusNum.Text)
                    cmd.Parameters.AddWithValue("@OldMobilePhone", If(oldMobilePhone Is Nothing, DirectCast(DBNull.Value, Object), oldMobilePhone))
                    cmd.Parameters.AddWithValue("@NewMobilePhone", If(newMobilePhone Is Nothing, DirectCast(DBNull.Value, Object), newMobilePhone))
                    cmd.Parameters.AddWithValue("@OldEmailAddress", If(oldEmailAddress Is Nothing, DirectCast(DBNull.Value, Object), oldEmailAddress))
                    cmd.Parameters.AddWithValue("@NewEmailAddress", If(newEmailAddress Is Nothing, DirectCast(DBNull.Value, Object), newEmailAddress))
                    cmd.Parameters.AddWithValue("@OldPreferredLanguage", If(oldLanguage Is Nothing, DirectCast(DBNull.Value, Object), oldLanguage))
                    cmd.Parameters.AddWithValue("@NewPreferredLanguage", If(newLanguage Is Nothing, DirectCast(DBNull.Value, Object), newLanguage))
                    cmd.Parameters.AddWithValue("@OldStatus", If(oldStatus Is Nothing, DirectCast(DBNull.Value, Object), oldStatus))
                    cmd.Parameters.AddWithValue("@NewStatus", If(newStatus Is Nothing, DirectCast(DBNull.Value, Object), newStatus))
    ...
    Public Sub ShowData()
        ...
        ' Store the initial states of the checkboxes

        chkAlertEmailOldState = chkAlertEmail.Checked

        chkAlertMobileOldState = chkAlertMobile.Checked
        ...
        ' Call another method to fetch the last update details
        GetLastUpdateDetails(txtCusNum.Text)
    ...
    Private Sub frmAlertServices_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        If Not IceUserAut.IceAutLst(458) And IceUserAut.IceAutLst(459) And IceUserAut.IceAutLst(290) Then
            HandleError(&H81000040, "User Operation Denied: Insufficient Authority (Update Enrolment)")
            ModalMsgBox("User Operation Denied: Insufficient authority to update enrolment", MsgBoxStyle.Exclamation, "ICE Security")
            Me.Close()
            Return
        ...
        If Not CheckDatabaseAccess() Then

            MessageBox.Show("Error: Unable to connect to the required database. Please check the connectivity and try again.", "Database Connectivity Error", MessageBoxButtons.OK, MessageBoxIcon.Error)

            Me.Close() ' Close the form if the user does not have access
    ...
    Private Function CheckDatabaseAccess() As Boolean
        ...
        Catch ex As Exception
            ...
            Return False ' If an exception occurs, return false indicating no access
    ...
    Private Sub btnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        If txtEmailAddressOld.Text = txtEmailAddress.Text AndAlso
           ...
           chkAlertEmailOldState = chkAlertEmail.Checked AndAlso
           chkAlertMobileOldState = chkAlertMobile.Checked AndAlso
        ...
        cmbStatus.Enabled = False
        cmbPreferedLanguage.Enabled = False
